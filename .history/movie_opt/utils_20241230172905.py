...
def get_time_base(video_path):
    ...
    # 获取 time_base 值
    time_base = result.stdout.decode().strip()
    
    if time_base:
        return time_base
    else:
        print(f"Error: Could not retrieve time_base from the video. FFmpeg Error: {result.stderr.decode()}")
        return None

# 示例使用
video_path = "C:\\Users\\luoruofeng\Desktop\\test\\视频片段\\每行中文视频-Lion King 2 1998-en@cn-3\\part01.fix.mp4"
...
def change_timescale(video_path, timescale=1000):
    ...
    result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(result)
    current_timescale = int(result.stdout.decode().strip())

    # 如果当前 timescale 已经是目标值，则无需修改
    if current_timescale == timescale:
        print(f"Timescale is already {timescale}, no changes made.")
        return
...
# 示例使用
# change_timescale("C:\\Users\\luoruofeng\\Desktop\\test\\视频片段\\每行中文视频-Lion King 2 1998-en@cn-3\\part01.mp4", 800)
