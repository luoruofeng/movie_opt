...
def filter_videos1(same_folder_index_videos: list):
    ...
    for video_path in same_folder_index_videos:
        ...
        if video_id in video_map:
            if "每行完整视频" in video_map[video_id] and "每行分段视频" in video_map[video_id]:
                # 如果同时包含 '每行完整视频' 和 '每行分段视频'，只保留 '每行完整视频'
...
result = filter_videos(same_folder_index_videos)
print(result)
...
def merge_mp4(args, folder_types):
    ...
    for folder_index in sorted(folder_indexs):
        ...
        # 如果同一视频 ID 同时出现在 '每行完整视频' 和 '每行分段视频' 中，则删除 '每行分段视频' 的项。
        filter_videos1 = filter_videos1(sorted_videos)
