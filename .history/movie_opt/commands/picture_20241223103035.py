...
def split_video(args):
    ...
    if srt_path.endswith(".srt"):
        ...
        screenshots_dir = os.path.join(os.path.dirname(video), "每行截图")
        os.makedirs(screenshots_dir, exist_ok=True)

        video_clips_dir = os.path.join(os.path.dirname(video), "每行视频")
        ...
        for content, end_time in splite_endtime_json.items():
            ...
            # ffmpeg -i "C:\Users\luoruofeng\Desktop\test\test_subtitled.mkv" -ss 4048.590 -to 4106.720 -accurate_seek "C:\Users\luoruofeng\Desktop\test\视频片段\Lion King 2 1998-en@cn-29.mkv"
            # ffmpeg -i "C:\Users\luoruofeng\Desktop\test\test_subtitled.mkv" -ss 4048.590 -force_key_frames expr:gte(t,T) -to 4106.720 -accurate_seek -c copy -avoid_negative_ts 1 "C:\Users\luoruofeng\Desktop\test\视频片段\Lion King 2 1998-en@cn-29.mkv"
            command = [
                "ffmpeg", "-y", "-i", video, "-ss", str(end_seconds), "-vframes", "1","-q:v", "2", screenshot_path
            ]
            # command = [
            #     "ffmpeg", "-i", video, "-g", "25", "-sc_threshold", "0", "-c:v", "libx264", "-c:a", "copy", screenshot_path
            # ]
            print(f"执行命令: {' '.join(command)}")
            subprocess.run(command)
