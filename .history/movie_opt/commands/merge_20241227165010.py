...
def merge_mp4(args):
    ...
    for folder_index in sorted(folder_indexs):
        ...
        same_index_videos = [
            ...
            if re.search(rf"-{folder_index}\\.mp4$", video)
        ]

        #视频的文件名格式为: folder_types-movie_name-folder_index, 用正在表达式找到视频名称中folder_index为folder_index变量的video，放入sasme_index_videos变量中

        # 按类型顺序处理视频文件
        for folder_type in folder_types:
            #循环folder_index的视频
            for video in same_index_videos:
                #根据video_index，folder_index正序拼接视频
