...
def merge1_diff_type(args,type):
    ...
    if type == 4:
        ...
        patter = "合并视频-最终"
        pattern = re.compile(rf"合并视频-(.*?)-{folder_index}-{folder_index}")
        movie_name = 

        output_dir = os.path.join(args.path, f"合并视频-{movie_name}")
        os.makedirs(output_dir, exist_ok=True)
        
        merge_list_path = os.path.join(args.path , "merge_list.txt")
        # 使用 ffmpeg 拼接视频
