...
def merge1_diff_type(args,type):
    ...
    if type == 4:
        if args.cnen_c is None:
            print("中英对照内容不能为空")
            return
        
        if args.ear_c is None:
            print("磨耳朵内容不能为空")
            return
        
        if args.follow_c is None:
            print("跟读内容不能为空")
            return
...
def merge_mp4(args, folder_types, video_type):
    ...
    for folder_index in sorted(folder_indexs):
        ...
        if output_video not in r_path_map:
            r_path_map[output_video] = []
        r_path_map[output_video].append(output_video)
    return r_path
