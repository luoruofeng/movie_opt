...
def merge1_diff_type(args,type):
    ...
    if type == 4:
        ...
        cnen_h = os.path.join(os.path.dirname(resource_filename(__name__,".")),'static', "中英对照.mp4")
        ear_h = os.path.join(os.path.dirname(resource_filename(__name__,".")),'static', "磨耳朵.mp4")
        follow_h = os.path.join(os.path.dirname(resource_filename(__name__,".")),'static', "跟读.mp4")
        ...
        for v in args.cnen_c:
            ...
            ear = get_file_by_suffix_number(args.ear_c,video_index)
            follow = get_file_by_suffix_number(args.follow_c,video_index)
            cnen = get_file_by_suffix_number(args.cnen_c,video_index)
