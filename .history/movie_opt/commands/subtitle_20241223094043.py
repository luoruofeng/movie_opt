...
def process_srt_segment_file(srt_path, output_folder, segment_second):
    ...
    for line in lines:
        ...
        start_time_sec = convert_srt_to_timedelta(start_time)
        end_time_sec = convert_srt_to_timedelta(end_time)

        # 判断是否需要分段
        if previous_end_time and (start_time_sec - previous_end_time > float(segment_second)):
            # 保存当前分段
