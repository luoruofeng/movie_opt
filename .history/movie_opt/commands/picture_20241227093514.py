...
def split_video(args):
    ...
    if srt_path.endswith(".srt"):
        ...
        video_clips_dir2 = os.path.join(os.path.dirname(video), "每行儿童发音视频-"+video_name)
        os.makedirs(video_clips_dir2, exist_ok=True)
        ...
        for content, attr in splite_endtime_json.items():
            ...
            if os.path.exists(content_voice):
                ...
                cn_voice_duration = get_mp3_duration_tinytag(content_voice)
            ...
            if os.path.exists(content_voice2):
                ding = os.path.join(os.path.dirname(resource_filename(__name__,".")),'static', "ding.mp3")
                merge_mp3_files(ding , content_voice2)
            ...
            print(f"生成视频片段: {content_voice}")

            os.remove(content_voice2)
            print(f"删除音频: {content_voice2}")

            os.remove(cn_voice)
            print(f"删除音频: {cn_voice}")
